...
void DelayCalculator::process(void) {
	...
	pDocument->getWholeDelay()->setPeriod(
		...
		getDelayPeriodFromTempoAndMutliplier(
				tempo,
				QUARTER_TO_WHOLE_MUTLT * alterationMultiplier
		)
	);
	...
	pDocument->getHalfDelay()->setPeriod(
		...
		getDelayPeriodFromTempoAndMutliplier(
				tempo,
				QUARTER_TO_HALF_MUTLT * alterationMultiplier
		)
	);
	...
	pDocument->getQuarterDelay()->setPeriod(
		...
		getDelayPeriodFromTempoAndMutliplier(
				tempo,
				QUARTER_TO_QUARTER_MUTLT * alterationMultiplier
		)
	);
	...
	pDocument->getEighthDelay()->setPeriod(
		...
		getDelayPeriodFromTempoAndMutliplier(
				tempo,
				QUARTER_TO_EIGHTH_MUTLT * alterationMultiplier
		)
	);
	...
	pDocument->getSixTeenthDelay()->setPeriod(
		...
		getDelayPeriodFromTempoAndMutliplier(
				tempo,
				QUARTER_TO_SIXTEENTH_MUTLT * alterationMultiplier
		)
	);
	...
	pDocument->getThirtySecondDelay()->setPeriod(
		...
		getDelayPeriodFromTempoAndMutliplier(
				tempo, QUARTER_TO_THIRTYSECOND_MUTLT * alterationMultiplier
		)
	);

...
void QrestMainWindow::on_actionPreferences_triggered() {
    ...
    if( dlg.exec() == QDialog::Accepted ) {
        ...
        /*
         * if preferred language has changed, we store it
         * unless user chose 'system default'
         */
        QString chosenLanguage = dlg.getChosenLanguage();

        if( Constants::LOCALE_SYSDEFAULT != chosenLanguage ) {

            Document::getInstance()->setLocale( chosenLanguage );

            Settings::getInstance()->getSettings().setValue(
                    Settings::UI_PREFERRED_LOCALE, chosenLanguage );
